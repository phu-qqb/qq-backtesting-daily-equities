...
def load_multiseries(path: str) -> pd.DataFrame:
    df = pd.read_csv(
        path,
        header=None,
...
hist = {f: load_multiseries(f) for f in files}
...
new_df = pd.read_csv('new_points.csv', index_col=0, parse_dates=True)
...
for label in ['A','H','I']:
    ...
    df_out.to_csv(f'{label}_updated.txt', header=False, index=False)
...
Series(all_ts.strftime('%Y-%m-%d %H:%M')).to_csv('D.txt',header=False,index=False)
print('D.txt généré')
